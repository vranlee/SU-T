...
class MFT25(ImageDataset):
    ...
    def __init__(self, root='datasets', **kwargs):
        ...
        data_dir = osp.join(self.data_dir, 'MFT25-ReID')    # 'fast_reid/datasets/MOT17-ReID'
        ...
        super(MFT25, self).__init__(train, query, gallery, **kwargs)
    ...
    def process_dir(self, dir_path, is_train=True):
        ...
        # <序号>_MOT17-<场景编号>-FRCNN

        # pattern = re.compile(r'([-\d]+)_([A-Z]+-\d+)')
        pattern = re.compile(r'([-\d]+)_([A-Z]+-\d+)_([-\d]+)_acc_data\.bmp')
        # 0000078_MSK-002_0000801_acc_data.bmp
        data = []
        for img_path in img_paths:

            pid, camid, frameid = pattern.search(img_path).groups()
            pid = int(pid)
            frameid = int(frameid)
            if pid == -1:
