...
class UKF(object):
    ...
    **Example**
    ...
    def __init__(self, dim_x, dim_z, dim_u=0, args=None, fx=None, hx=None):
        if dim_x < 1:
        ...
        self.sigmas_f = zeros((self.n_sigma, self.dim_x))
    ...
    def _calculate_sigma_points(self):
        """Calculate sigma points using the current state and covariance."""
        n = self.dim_x
        lambda_ = self._alpha ** 2 * (n + self._kappa) - n

        # Ensure P is a symmetric positive definite matrix
        P = self.P.copy()
        P = (P + P.T) / 2  # Force symmetry

        try:
            # First, try direct Cholesky decomposition
            U = cholesky((lambda_ + n) * P)
        except np.linalg.LinAlgError:
            # If it fails, attempt to fix the matrix via eigenvalue decomposition
            try:
                eigvals, eigvecs = np.linalg.eigh(P)
                # Set small or negative eigenvalues to positive values
                threshold = 1e-6 * max(np.max(np.abs(eigvals)), 1e-10)
                ...
                # If it still fails, use incremental diagonal terms
                d = 1e-4
                max_attempts = 10
                attempts = 0
                ...
                if attempts == max_attempts:
                    raise ValueError("Unable to convert covariance matrix to positive definite.")
        ...
        for i in range(n):
            sigmas[i + 1] = self.x.flatten() + U[i]
            sigmas[n + i + 1] = self.x.flatten() - U[i]

        return sigmas

    def predict(self, u=None, B=None, F=None, Q=None):
        """
        Predict the next state (prior) using the Unscented Kalman Filter state propagation equations.
        ...
        sigmas = self._calculate_sigma_points()

        # Propagate sigma points
        for i in range(self.n_sigma):
            if self.fx is not None:
                # If a nonlinear function is provided, use it
                self.sigmas_f[i] = self.fx(sigmas[i], u)
            else:
                # Otherwise use the linear function F*x + B*u
                if B is not None and u is not None:
                    self.sigmas_f[i] = dot(F, sigmas[i]) + dot(B, u)
                else:
                    self.sigmas_f[i] = dot(F, sigmas[i])

        # Compute prior mean
        self.x = np.zeros((self.dim_x, 1))
        for i in range(self.n_sigma):
            self.x += self.Wm[i] * self.sigmas_f[i].reshape(-1, 1)

        # Compute prior covariance
        self.P = Q.copy()  # Start with Q
        for i in range(self.n_sigma):
            y = self.sigmas_f[i].reshape(-1, 1) - self.x
    ...
    def update(self, z, R=None, H=None):
        ...
        If z is None, no computation is performed. However, x_post and P_post
        will be updated with the prior (x_prior, P_prior), and self.z will be set to None.

        Parameters
        ----------
        z : (dim_z, 1): array_like
            Measurement for this update. If dim_z is 1, z can be a scalar,
        ...
        if H is None:
            raise ValueError("Measurement matrix H cannot be None")
        ...
        if z is None:
            if self.observed:
                """
                    No observation received, so freeze current parameters
                    for potential future online smoothing.
                """
                self.freeze()
            self.observed = False
            self.z = np.array([[None] * self.dim_z]).T
        ...
        if not self.observed:
            """
                Observation received, use online smoothing to update parameters
            """
            self.unfreeze()
        self.observed = True
        ...
        sigmas_h = np.zeros((self.n_sigma, self.dim_z))
        ...
        for i in range(self.n_sigma):
            if self.hx is not None:
                ...
                # Otherwise use the linear function H*x
                sigmas_h[i] = dot(H, self.sigmas_f[i])

        # Compute measurement prediction
        zp = np.zeros((self.dim_z, 1))
        for i in range(self.n_sigma):
            zp += self.Wm[i] * sigmas_h[i].reshape(-1, 1)

        # Compute measurement covariance
        self.S = R.copy()  # Start with R
        for i in range(self.n_sigma):
            y = sigmas_h[i].reshape(-1, 1) - zp
            self.S += self.Wc[i] * np.outer(y, y)

        # Compute cross covariance
        Pxz = np.zeros((self.dim_x, self.dim_z))
        for i in range(self.n_sigma):
            dx = self.sigmas_f[i].reshape(-1, 1) - self.x
            dz = sigmas_h[i].reshape(-1, 1) - zp
    ...
    def rts_smoother(self, Xs, Ps, Fs=None, Qs=None, inv=np.linalg.inv):
        ...
        computed by a Kalman filter. The usual input would come from the output of
        ...
        inv : function, default numpy.linalg.inv
            If you prefer another inverse function, such as the Moore-Penrose
            pseudo-inverse, set it to: kf.inv = np.linalg.pinv
        ...
        for k in range(n - 2, -1, -1):
            # In UKF, we need to use the nonlinear state transition function.
            # This is a simplified version of the RTS smoother using a linear approximation.
    ...
    def get_prediction(self, u=None, B=None, F=None, Q=None):
        """
        Predicts the next state (prior) using the Kalman filter state propagation
        ...
        u : np.array, default 0
            Optional control vector.
        B : np.array(dim_x, dim_u), or None
            Optional control transition matrix; a value of None will cause the filter
            to use `self.B`.
        F : np.array(dim_x, dim_x), or None
            Optional state transition matrix; a value of None will cause the filter
            to use `self.F`.
        Q : np.array(dim_x, dim_x), scalar, or None
            Optional process noise matrix; a value of None will cause the filter
            to use `self.Q`.

        Returns
        -------
        (x, P) : tuple
            Predicted state vector and covariance array.
        """
        ...
        sigmas_f = np.zeros((self.n_sigma, self.dim_x))
        for i in range(self.n_sigma):
            if self.fx is not None:
                sigmas_f[i] = self.fx(sigmas[i], u)
            else:
                if B is not None and u is not None:
                    sigmas_f[i] = dot(F, sigmas[i]) + dot(B, u)
                else:
                    sigmas_f[i] = dot(F, sigmas[i])


        x = np.zeros((self.dim_x, 1))
        for i in range(self.n_sigma):
            x += self.Wm[i] * sigmas_f[i].reshape(-1, 1)


        P = Q.copy()  
        for i in range(self.n_sigma):
            y = sigmas_f[i].reshape(-1, 1) - x
